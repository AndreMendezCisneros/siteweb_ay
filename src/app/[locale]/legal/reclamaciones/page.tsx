...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return { title: getDict(locale).pages.legalReclamaciones.metaTitle };
}

export default async function ReclamacionesPage({ params }: Props) {
  const { locale } = await params;
  const t = getDict(locale).pages.legalReclamaciones;
  return (
    <>
